...
// We saw this for example with worker pools. There are a few other options for managing
// state though. Here we'll look at using the `sync/atomic` package for atomic counters
...
func main() {
	// We'll use an unsigned integer to reporesent our (always-positive) counter.
	var ops uint64
	...
	// Reading atomics safely while they are being updated is also possible, using functions
...
// because the goroutines would interfere with each other. Moreover, we'd get data race
// failures when running with the `-race` flag.

// go run atomic-couters.go
