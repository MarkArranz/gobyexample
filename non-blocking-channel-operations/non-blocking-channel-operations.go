// Basic sends and receives on channels are blocking. However, we can use `select` with
// a `default` clause to implement non-blocking sends, receives, and even non-blocking
// multi-way `select`.
package main
...
func main() {
	...
	// A non-blocking send work similarly. Here `msg` cannot be sent to the `messages`
	...
	// non-blocking select. Here we attempt non-blocking receives on both `message` and
	// `signales`.
	select {
	case msg := <-messages:
		fmt.Println("received message", msg)
