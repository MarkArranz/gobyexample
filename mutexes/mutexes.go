...
func main() {
	...
	// Here we start 100 goroutines to execute repeated reads against the state, once per
	// millisecond in each goroutine.
	for r := 0; r < 100; r++ {
		go func() {
			total := 0
			for {
				// For each read we pick a key to access, `Lock()` the `mutex` to ensure
				// exclusive access to the `state`, read the value at the chosen key,
	...
	// We'll also start 10 goroutines to simulate writes, using the same pattern
	// we did for reads.
	for w := 0; w < 10; w++ {
		go func() {
			for {
...
// Running the program shows that we executed about 70,000 total operations against our
