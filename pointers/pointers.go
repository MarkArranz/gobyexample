...
// We'll show how pointers work in contrast to values with 2 functions: `zeroval` and
// `zeroptr`. `zeroval` has an `int` parameter, so arguments will be passed to it by
// value. `zeroval` will get a copy of 	`ival` distinct from the one in the calling
// function.
...
func main() {
	...
	// Pointers can be printed to.
	fmt.Println("pointer:", &i)
}

// `zeroval` doesn't change the `i` in `main`, but `zeroptr` does because it has a
// reference to the memory address for that variable.
