...
// `main()` goroutine to a worker goroutine. When we have no more jobs for the worker
...
func main() {
	...
	// will be `false` if `jobs` has been `closed` and all values in the channel have
	// already been received. We use this to notify on `done` when we've worked all our
	// jobs.
	go func() {
		for {
			j, more := <-jobs
